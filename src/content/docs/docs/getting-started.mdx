...
Tinyauth sets a cookie for the parent domain of the application URL, unless `TINYAUTH_AUTH_SINGLECOOKIEDOMAIN=true` is set. For example, if the application URL is `http://tinyauth.example.com`, the cookie is set for `.example.com`, enabling authentication across all subdomains. Below is an example of an ideal domain structure:
...
:::caution
  ...
  `TINYAUTH_AUTH_SINGLECOOKIEDOMAIN=true` is set. Subdomains (e.g.,
